...
> IAM 是「是什么」，IDaaS 是「怎么交付」。如同数据库 vs RDS——你不会问「选数据库还是 RDS」，你问的是「用什么 RDS 实例来做数据库」。
...
简单说：**IAM 是能力范畴，IDaaS 是交付模式**。IAM 告诉你「要管理谁可以访问什么」，IDaaS 告诉你「把这个能力做成云服务」。详细对比见 [IDaaS 完全指南]({{< relref "what-is-idaas" >}})。
