...
| 版本 | 状态 | 建议 |
|------|------|------|
| Keycloak 26.7.4 | 官方发布页当前可见的 26.7 补丁版本（2026-09-16 发布，6 项安全修复；其中 stateless + MySQL/MariaDB 的单次性凭据重放防护失效必须优先处理） | 新项目优先在目标拓扑上验证后采用；生产环境应尽快升级 |
| Keycloak 26.x 的其他维护版本 | 需按具体补丁版本核对支持状态 | 已有项目不要只因小版本号变化就盲目升级，先读升级说明 |
| Keycloak 25.x 及更早 | 不应作为新项目基线 | 规划升级并在测试环境验证兼容性 |

> **版本判断边界**：截至 2026-09-17，Keycloak 官方 GitHub Release 页面可见的最新发布标签为 `26.7.4`（2026-09-16 发布）。版本会继续变化，生产升级前应重新核对 [官方发布页](https://github.com/keycloak/keycloak/releases)、[升级指南](https://www.keycloak.org/docs/latest/upgrading/index.html) 和 [反向代理配置](https://www.keycloak.org/server/reverseproxy)。升级仍需结合数据库迁移、自定义 SPI、反向代理和扩展兼容性验证；“最新”不是回滚方案。详见 [Keycloak 26.7 更新速览]({{< relref "../solution-blogs/keycloak-26-7-whats-new" >}}) 与 [Keycloak 26.7.4 安全补丁解读]({{< relref "../solution-blogs/keycloak-26-7-4-security-patch" >}})、[Keycloak 26.7.3 安全补丁解读]({{< relref "../solution-blogs/keycloak-26-7-3-security-patch" >}})。
