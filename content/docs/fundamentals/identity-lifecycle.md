...
SCIM（System for Cross-domain Identity Management）是专门用于跨域身份管理的标准协议。将在第9章详细讨论。
...
- HR 系统连接器（Workday、PeopleSoft）
- 目录连接器（AD、LDAP）
- 数据库连接器（MySQL、PostgreSQL）
- 云应用连接器（Office 365、Salesforce）
- 自定义脚本连接器
...
3. **及时撤销**：离职员工账号应在离职生效后 24 小时内完成吊销。
