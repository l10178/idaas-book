...
| 登录后反复跳转 / `csrf cookie not found` | `redirect_url`、Ingress `auth-signin`、Cookie Domain / SameSite 与实际访问域名不一致 | `redirect_url` 固定为外部入口 `https://app.example.com/oauth2/callback`；Ingress 使用 `$host` 与 `$escaped_request_uri`；跨子域共享时再设置 `--cookie-domain=.example.com`。反向代理还必须覆盖而不是盲目追加 `X-Forwarded-*`，并限制 Keycloak 只接受可信代理；完整排查见 [Keycloak 重定向循环与 401 排错指南]({{< relref "docs/solution-blogs/keycloak-redirect-loop-troubleshooting.md" >}})。 |
