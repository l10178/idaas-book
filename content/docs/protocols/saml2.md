...
| 签名方式 | XML DSig | JWS |
| 加密方式 | XML Enc | JWE |
| 绑定协议 | 重定向、POST、Artifact 等 | 仅 HTTP |
| 移动端支持 | 差 | 好 |
| SPA 支持 | 差 | 好 |
| 复杂度 | 高 | 低 |
| 行业采用 | 传统企业 | 现代应用 |
...
3. **Authorization Decision Statement**：用户是否有权做某事（已过时，不常用）
...
Shibboleth 是教育和研究领域的 SAML 实现标准。eduroam、InCommon 等联邦网络使用 Shibboleth 实现跨机构身份认证。

### 中国场景（CAS + SAML）

Apereo CAS 同时支持 SAML 和 OIDC，在高校和政务场景中广泛使用。我们将在第15章详细讨论 CAS。
...
Keycloak 的 Identity Brokering 实现中支持这种转换，我们将在第14章详细讨论。
...
7. **签名验证前不要解析 XML**：防止 XML 签名包装攻击（XML Signature Wrapping）。
