...
**nonce**：从授权请求中传入，在 ID Token 中返回，用于防 CSRF 和重放攻击。

**amr（Authentication Methods Reference）**：用户认证的方式：
- `pwd` — 密码
- `otp` — 一次性密码
- `mfa` — 多因素认证（多个认证方式同时使用）
- `swk` — 软件加密密钥
...
```
OAuth 2.0 授权请求（仅获取 Access Token）：
  response_type=token
  scope=photos:read
...
Session Management RFC 定义了基于 iframe 的会话状态检测，但由于浏览器的第三方 Cookie 限制（Safari ITP、Firefox ETP），这种方法已基本不可用。建议使用 Back-Channel Logout 和短 Session 作为替代方案。
...
□ iss 是否匹配预期的 issuer？
□ aud 是否包含自己的 client_id？
□ exp 是否未过期？
□ iat 是否在合理范围内（不能是未来时间）？
□ nonce 是否与请求时的一致？（如果请求中使用了）
□ 如果是 Authorization Code Flow，验证 at_hash 和 c_hash？
```
