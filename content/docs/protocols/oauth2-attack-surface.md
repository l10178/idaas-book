---
title: "OAuth 2.0 攻击面与防护 — IAM 安全威胁分析与防御指南 | IDaaS Book"
description: "IAM OAuth 2.0 五大攻击面完整分析：Redirect URI 劫持、CSRF/State 伪造、授权码拦截、Mix-Up 攻击与 Token 泄露。附 IAM 安全防御措施、Mermaid 图解与生产检查清单"
...
seo:
  title: "OAuth 2.0 攻击面与防护 — IAM 安全威胁分析与防御指南 | IDaaS Book"
  description: "IAM OAuth 2.0 五大攻击面（Redirect URI 劫持、CSRF、授权码拦截、Mix-Up Attack、Token 泄露）的 Mermaid 图解与防护措施。涵盖 PKCE、DPoP、mTLS、state 参数和 RFC 9207。"
---
...
| 3 | 授权码拦截 | 无 PKCE 时授权码可被恶意 App 截获 | PKCE（code_challenge + code_verifier） | RFC 7636、RFC 9700 |
| 4 | Mix-Up Attack | 客户端未校验授权响应的 issuer / 端点一致性 | 验证授权响应中的 `iss` + 端点一致性 | RFC 9207、RFC 9700 |
...
- [ ] **iss 验证**：客户端验证授权响应中的 `iss`（或 OIDC 的等价 issuer 绑定）与预期一致
...
| "PKCE 只是给移动 App 用的" | RFC 9700 要求授权码客户端使用 PKCE；Web 应用、SPA、移动端、CLI 都应纳入基线 |
...
| "客户端验证 iss = 多此一举" | 多授权服务器场景必须绑定授权响应与预期 issuer；RFC 9207 定义了 `iss` 参数，RFC 9700 将其列为推荐防护路径 |
...
截至 2026 年 7 月，OAuth 2.1（draft-ietf-oauth-v2-1）尚未成为 RFC。已经正式发布的 RFC 9700（OAuth 2.0 Security BCP）给出了独立可执行的安全要求：客户端不应使用 Implicit Grant，ROPC 不得使用，授权码流程应采用 PKCE；不要把草案中的措辞或未来状态写成已发布规范。
...
- [SAML 2.0 攻击面与安全防护]({{< relref "saml2-attack-surface" >}})：IAM 身份联邦的另一大支柱 SAML 的安全分析——XSW、断言重放、XXE 与元数据伪造
