...
lastmod: 2026-07-14T23:05:00+08:00
draft: false
weight: 27
menu:
  docs:
...
## 9.1 为什么需要 SCIM？——IAM 用户生命周期的标准化

在企业 IAM（身份与访问管理）体系中，OAuth 2.0 和 OIDC 解决了认证和授权问题，SAML 解决了联邦 SSO 问题，但它们都没有回答一个 IAM 核心问题：如何标准化地创建、更新和删除用户？SCIM 正是 IAM 用户生命周期管理的标准答案——它把"入职创建账号 → 调岗修改权限 → 离职回收身份"这一整套 IAM 流程变成了标准化的 REST API。
...
sequenceDiagram
    ...
    IDaaS->>IDaaS: 将用户标记为 inactive<br/>后续认证/授权按实现处理
...
2. **为什么不能把 `active=false` 当成 Token 吊销？** SCIM 只改变目标系统中的资源状态；RFC 7644 没有规定它必须销毁 OIDC/OAuth 会话或已经签发的 Token。连接器应明确实现“停用后拒绝新登录、撤销会话/刷新令牌、缩短 Access Token 暴露窗口”等动作，并用应用侧验证确认已生效。

3. **为什么用 externalId 而非 userName 做上下游关联？** `userName` 可能随改名/婚变而变化，`externalId`（如工号 `EMP-12345`）在员工生命周期内稳定不变。SCIM 规范建议 `externalId` 作为跨系统主键。

4. **为什么图中展示了管理员介入的步骤？** SCIM 自动化了大部分重复操作，但 DELETE 这种不可逆操作需要人工确认。图中 Leaver 阶段的 `Admin→IDaaS` 步骤可以是手动审批、也可以是“停用 N 天后自动删除”的定时任务。
...
- PATCH `/Users/{id}` `{"active": false}` — 将用户标记为非活动；是否同时撤销登录会话、刷新令牌和 API Token，取决于目标系统的实现
- DELETE `/Users/{id}` — 删除用户
...
> SCIM 的 `active` 字段是资源状态，不是跨协议的全局“立即踢下线”开关。建议的离职流程：先 `PATCH active=false`，由目标系统明确执行新登录阻断和会话/令牌处置，保留数据供审计和交接；确认无误后再 `DELETE`。不要直接从 `active: true` 跳到 `DELETE`。

### 离职验证清单：不要只看 SCIM 返回 200

```bash
# 1. 确认 SCIM 资源状态
curl -sS -H "Authorization: Bearer $PROVISION_TOKEN" \
  "https://scim.example.com/v2/Users/$PROVISION_USER_ID" | jq '{id,userName,active,meta}'

# 2. 用该用户的会话访问一个受保护资源；预期是 401/403，
#    而不是仅仅得到一次成功的 SCIM PATCH 响应
curl -i -H "Authorization: Bearer $SESSION_TOKEN" \
  https://app.example.com/api/me
```

如果第二步仍返回 200，不要把问题归咎于“SCIM 延迟”就结束排查：检查目标应用是否只在登录时读取 `active`、是否缓存了用户状态，以及 IDaaS 是否真正撤销了 Refresh Token 和应用会话。这个验证应纳入离职演练和回归测试；否则自动化只是把“忘记回收权限”变成了更快的脚本。
