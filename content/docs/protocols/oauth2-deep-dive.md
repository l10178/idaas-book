...
```
需要的是？
 ├─ 用户授权第三方访问 ──→ 授权码 + PKCE（默认选择）
 ├─ 服务端到服务端 ──→ 客户端凭证模式
 ├─ 输入受限设备 ──→ 设备授权模式
 ├─ 高性能要求的 M2M ──→ 客户端凭证模式（可结合 JWT Profile / RFC 7523）
 ├─ 工作负载身份联邦 ──→ JWT Bearer（RFC 7523）
 └─ 你有旧系统在用隐式/密码模式 ──→ 尽快迁移到授权码+PKCE
```
...
OAuth 2.1 是 IETF 草案（draft-ietf-oauth-v2-1），截至本稿时尚未成为 RFC，且其安全要求已通过 **RFC 9700（OAuth 2.0 Security BCP）** 落地。它融合了 OAuth 2.0 的最佳实践和扩展 RFC，主要变化：

1. **不推荐隐式模式和密码模式**（OAuth 2.0 Security BCP / RFC 9700 已明确）
2. **PKCE 对所有客户端强制**
3. **Refresh Token 仅用于机密客户端**（SPA 用 BFF）
4. **精确的 redirect_uri 匹配**
5. **推荐使用 DPoP 进行 Token 绑定**
...
- [OAuth 2.0 授权码流程与 PKCE 完整图解]({{< relref "docs/protocols/oauth2-authorization-code-pkce.md" >}})：包含完整 Mermaid 时序图的授权码流程详解、PKCE 的密码学原理、OAuth 2.1 变更对比和常见攻击面分析。
- [OpenID Connect]({{< relref "openid-connect.md" >}})：理解 OAuth 2.0 后，下一步是学习基于它的身份认证层协议。
