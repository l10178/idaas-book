...
**这是最安全、最广泛使用的模式**。核心特征是用户浏览器不直接接触 Access Token——它只拿到一个一次性授权码（authorization code），由后端服务器用这个 code 去换 token，用户密码全程不经过客户端后端。

```mermaid
sequenceDiagram
    actor User as 用户 (Resource Owner)
    participant Browser as 浏览器 / User-Agent
    ...
    User->>Browser: 1. 点击"用 OAuth 登录"
    Browser->>Client: 2. 发起访问请求
    Client->>Browser: 3. 302 重定向至 /authorize<br/>response_type=code<br/>client_id=xxx<br/>redirect_uri=yyy<br/>scope=zzz<br/>state=random

    Browser->>AuthServer: 4. GET /authorize?...
    AuthServer->>Browser: 5. 展示登录页面
    User->>Browser: 6. 输入凭证登录
    Browser->>AuthServer: 7. POST 登录凭证
    AuthServer->>Browser: 8. 展示授权确认页面<br/>（"允许 xxx 访问你的照片？"）
    User->>Browser: 9. 确认授权
    Browser->>AuthServer: 10. POST 授权确认

    AuthServer->>Browser: 11. 302 重定向至 redirect_uri<br/>?code=AUTHORIZATION_CODE<br/>&state=random

    Browser->>Client: 12. GET /callback?code=...&state=...
    Client->>Client: 13. 验证 state 匹配（防 CSRF）
    Client->>AuthServer: 14. POST /token<br/>grant_type=authorization_code<br/>code=AUTHORIZATION_CODE<br/>code_verifier=PKCE_VERIFIER<br/>client_id=xxx<br/>client_secret=xxx

    AuthServer->>AuthServer: 15. 验证 code、client_secret、<br/>code_verifier、redirect_uri
    AuthServer->>Client: 16. 返回 access_token + refresh_token

    Client->>ResourceServer: 17. GET /api/resource<br/>Authorization: Bearer ACCESS_TOKEN
    ResourceServer->>ResourceServer: 18. 验证 token（本地验签<br/>或 Introspection 端点）
    ResourceServer->>Client: 19. 返回受保护资源
    Client->>Browser: 20. 渲染页面/返回数据
    Browser->>User: 21. 看到目标内容
```
...
适用于输入受限的设备（智能电视、打印机、IoT 设备）。详细原理、Keycloak 配置、CLI 集成示例和排错指南见 [OAuth 2.0 设备授权流程实战指南]({{< relref "docs/solution-blogs/oauth2-device-authorization-grant.md" >}})。
...
- [JWT 深入解读]({{< relref "jwt-deep-dive" >}})：OAuth Token 的底层载体——JWT 结构、声明、签名验证、JWS/JWE/JWK 与常见攻击向量
- [OAuth 2.0 攻击面与防护深度图解]({{< relref "docs/protocols/oauth2-attack-surface.md" >}})：五大攻击面的完整分析，含 Mermaid 攻击流程图和防护措施
- [Token Introspection 实践]({{< relref "docs/solution-blogs/oauth2-token-introspection-guide.md" >}})：API 网关/后端如何远程验证 Token（RFC 7662），含 Nginx auth_request 和缓存策略
