...
需要的是？
 ...
 ├─ 服务端到服务端 ──→ 客户端凭证模式
 ├─ 输入受限设备 ──→ 设备授权模式
 ├─ 高性能要求的 M2M ──→ JWT Bearer Token + 客户端凭证
...
- 有效期较长（数小时到数天）
- 一次性使用，用完即替换新 Refresh Token（Refresh Token Rotation）
- 泄露后可以全局吊销
...
| 混用攻击 | 混合不同客户端的 token | audience 验证 |
| Refresh Token 窃取 | 获取长期有效的 RT | RT Rotation + 自动检测重用 |

## 5.7 OAuth 2.1

OAuth 2.1 是正在制定中的统一规范，融合了 OAuth 2.0 的最佳实践和扩展 RFC。主要变化：
