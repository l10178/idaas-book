...
### 场景四：管理控制台进不去（管理员访问丢失）

**症状**：所有服务正常、用户能在应用侧登录，但没有人能登录管理控制台——管理员密码丢失、管理员账号被误删，或密码正确却卡在 OTP/passwordless 步骤。

**应急步骤**：
1. 先确认不是暴力破解检测的临时锁定：短时间连续失败会被临时拒绝，这不是账号丢失，等待锁定窗口即可。
2. 确认 master realm 里还有哪些管理员账号可用（其它运维同事手上的凭据、受管密码库）。
3. 全都不可用时走停服恢复：所有节点降到 0 副本 → 用与服务器相同的 db 选项执行 `kc.sh bootstrap-admin user`（或强化认证场景下的 `bootstrap-admin service`）→ 用临时账号建正式管理员并重置原凭据 → 删除临时账号。

完整命令、`--optimized` 与 db 选项前提、常见错误对照见 [Keycloak 管理员账号进不去：bootstrap-admin 恢复与 26.x 变量变更]({{< relref "blog/keycloak-admin-account-recovery" >}})。

**预防**：至少保留两个长期管理员账号、凭据入密码库、强化认证配置备用凭据——否则每次故障都必须付出一次停服代价。
