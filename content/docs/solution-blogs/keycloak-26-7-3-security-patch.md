...
> **补丁版本更新（2026-09-17）**：26.7.4（2026-09-16）已发布，是当前最新的补丁版本。本文记录的 26.7.3 修复条目仍然适用，但升级目标应改为 26.7.4——它修复的 6 项 CVE 与本文条目不重叠，其中 `CVE-2026-90997` 会让 stateless 模式下的单次性凭据重放防护失效。见 [Keycloak 26.7.4 安全补丁解读]({{< relref "keycloak-26-7-4-security-patch" >}})。

26.7.3 于 **2026-08-31** 发布，是当时 26.7 系列最新的补丁版本。它不引入新特性，但修复量级值得单独说明：官方 release note 列出 **20 项安全修复（CVE）、6 项弱点修复和 19 项缺陷修复**，合计 45 项。其中 **10 项直接标注 `admin/fine-grained-permissions`（FGAP v2）**，另有 1 项标注 `admin/rbac`。
