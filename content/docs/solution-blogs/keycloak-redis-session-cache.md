...
> **版本说明**：Keycloak 26.7.x（26.7.0 于 2026-07-09 发布，26.7.4 于 2026-09-16 发布）引入了免外部缓存的多集群 HA 模式（特性开关 `stateless`）。如果你刚从 26.7 起步且只用单集群，可以跳过本文。如果你在 25.x / 早期 26.x，或者需要跨集群/跨数据中心共享 Session，Redis 外挂仍然是经过生产验证的选择。
