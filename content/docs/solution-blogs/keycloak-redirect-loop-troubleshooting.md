...
**信任边界**：`X-Forwarded-*` 不是客户端输入就可信。TLS 终结层应先删除或覆盖来自上游的同名 Header，再写入它实际观察到的值；Keycloak 只应接收来自受信代理的请求。若链路中有多层代理，逐层确认哪一层负责覆盖 Header，不要仅在应用日志里看到一个值就认为它可靠。
...
| 一个浏览器正常、另一个异常 | 浏览器对第三方 Cookie、跟踪保护或站点上下文的处理差异 | SameSite、Cookie Domain、Secure 以及浏览器开发者工具中的阻止原因 | 先看 Network/Storage 面板中的 Cookie 阻止理由，不要假定某个浏览器固定使用 Strict |
