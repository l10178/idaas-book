...
- **负载均衡**：前面挂 LB（Nginx/HAProxy/云 LB），只把就绪节点加入后端。不要把 `ip_hash` 当成高可用方案；它会降低故障切换和扩容效果。是否启用会话亲和性，应以所用 IAM 产品的缓存模型和压测结果为准；Keycloak 官方文档明确建议生产部署考虑 session affinity，以减少认证会话在节点间转移的成本，但这不是“有亲和性就不需要集群”的替代品。
