...
一个容易被忽略的边界是：协议选型不能替代授权边界设计。用 OIDC 登录并不等于应用已经完成 API 授权；网关放行后，资源服务器仍需验证 Access Token 的 `iss`、`aud`、有效期和权限。以 IAM 网关为例，见 [Keycloak + oauth2-proxy 的认证与后端校验边界]({{< relref "../solution-blogs/keycloak-oauth2-proxy" >}})。
...
## 选型落地检查清单：先验证边界，再接入应用

协议名称写进架构图不等于接入完成。下面这组检查适合放在 IAM 应用接入的验收单里；它把“能登录”与“能安全运行”分开，避免上线后才发现协议选对了、信任边界却没落地。

| 检查项 | 最小验收动作 | 不通过时的处理 |
|--------|--------------|----------------|
| OIDC 发现与签发者 | 从应用实际使用的 issuer URL 读取 `/.well-known/openid-configuration`，确认 `authorization_endpoint`、`token_endpoint` 和 `jwks_uri` 属于同一个发行者 | 不要先改应用代码；先修正 issuer、反向代理的外部 URL 或 TLS 终止配置 |
| 令牌用途 | 用测试令牌分别验证 `iss`、`aud`、`exp` 和权限声明；资源服务不能只验签名 | 回退到旧客户端或网关规则，同时保留失败令牌的 `iss/aud`（不要记录完整令牌） |
| 回调与 PKCE | 用一次成功授权和一次错误 `redirect_uri`/`code_verifier` 的请求做负向测试 | 禁止放宽回调地址或关闭 PKCE 作为“临时修复” |
| SAML 元数据 | 校验 Entity ID、ACS URL、签名证书有效期，并用过期证书演练拒绝行为 | 保留旧证书和旧 SP 配置，先完成证书轮换，再切换生产信任关系 |
| SCIM 生命周期 | 用测试用户演练创建、属性更新、`active=false` 和重复请求；确认下游重试不会产生重复账号 | 暂停自动推送，进入人工补偿队列；不要直接删除生产账号 |

OIDC 的发现文档定义了端点位置，OAuth 资源服务的令牌校验则必须结合具体授权服务器的约定；因此“发现接口返回 200”只能证明配置可读，不能证明应用已经完成授权校验。SCIM 的生命周期验证同样不能只看 HTTP 成功码，还要核对下游最终状态。

### 一次可回滚的接入顺序

1. 先创建新客户端或新 SP，保留旧客户端不动；新配置只允许一个测试用户或测试组。
2. 完成上表中的成功和负向测试，并记录 issuer、受众、回调地址、证书指纹和下游账号状态。
3. 小范围切换流量，观察登录失败率、回调错误、令牌校验失败和 SCIM 重试队列。
4. 出现异常时，将应用的认证入口切回旧客户端/旧 SP，暂停 SCIM 推送，保留新配置供定位；不要通过接受任意 `aud`、任意 `redirect_uri` 或跳过签名校验来止血。

这份清单的关键不是增加文档，而是为协议选型补上**可证明的退出路径**：每种协议都要有成功条件、失败信号和回滚动作。
...
建议先完成[协议选型]({{< relref "docs/protocols/_index.md" >}})（本章），再做[架构设计]({{< relref "docs/advanced-topics/iam-architecture-design.md" >}})，最后确定[授权模型]({{< relref "docs/advanced-topics/authorization-models.md" >}})。这样可以先锁定外部接口契约，再把高可用、缓存和权限决策放进架构设计，减少后期返工。

如果你的 IAM 平台需要同时支持多种协议（如 OIDC + SAML），选型完成后建议直接参考 [IAM 多协议集成实战]({{< relref "docs/solution-blogs/iam-multi-protocol-integration.md" >}})，其中包含跨协议会话统一、属性映射和常见排错。
