...
- 密码历史检查（不能与最近 5 次相同）
- 但不要强制每 90 天换密码（NIST SP 800-63B 已取消此建议）——对管理员可设定期更换（等保 2.0 要求），普通用户不设；完整分层方案见 [IAM 密码策略实战]({{< relref "../solution-blogs/iam-password-policy-guide" >}})
- 泄露密码检测（Keycloak `passwordBlacklist` 策略，配置方法见 [IAM 密码策略实战]({{< relref "../solution-blogs/iam-password-policy-guide" >}})）
- 只在怀疑凭证泄露时要求更改
...
- [Passkey / WebAuthn / FIDO2 IAM 企业落地指南]({{< relref "../solution-blogs/keycloak-passkey-webauthn" >}})：FIDO2 协议 Mermaid 流程图解、Keycloak Passkey 配置与 IAM 企业恢复策略
