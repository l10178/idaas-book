---
title: "第22章：IAM 高可用与 IDaaS 性能扩展 — 缓存策略、集群扩展与容量规划 | IDaaS Book"
description: "IAM 高可用架构与 IDaaS 性能扩展指南：深入数据库优化、分布式缓存策略、集群水平扩展与科学的负载测试方案，涵盖 IAM 集群部署和容量规划"
...
- [Keycloak 高可用集群部署与灾难恢复实战]({{< relref "../solution-blogs/keycloak-ha-dr" >}}) — 多节点集群、JGroups 发现、InfiniSpan 缓存与灾难恢复流程
...
详见 [IAM 架构设计指南]({{< relref "docs/advanced-topics/iam-architecture-design.md" >}}) 中的高可用章节。
...
优先用监控（Prometheus + Grafana）定位瓶颈，而不是凭感觉加机器。详见 [Keycloak Prometheus 指标监控]({{< relref "../solution-blogs/keycloak-prometheus-metrics" >}})。
...
详见 [Keycloak 集群缓存调优]({{< relref "../solution-blogs/keycloak-cluster-cache-tuning" >}})。
