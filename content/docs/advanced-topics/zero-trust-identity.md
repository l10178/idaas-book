...
零信任架构里最容易被混淆的一点是：**资源服务器验证 JWT 签名，只能证明令牌由可信签发方签发且仍在 `exp` 内；它不会自动知道用户会话是否刚被注销，也不会重新评估设备风险。** 因此，“每次请求验签”不等于“每次请求都向 IAM 重新询问”。

| 做法 | 能解决什么 | 代价与边界 |
|------|------------|------------|
| 本地校验 JWT 签名、`iss`、`aud`、`exp` | 低延迟、IdP 短暂不可用时仍可验 token | 吊销和权限变更通常要等 token 过期 |
