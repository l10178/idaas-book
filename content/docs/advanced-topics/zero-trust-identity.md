...
- **PDP**（Policy Decision Point）：做出访问决策
- **PEP**（Policy Enforcement Point）：执行访问决策
- **PIP**（Policy Information Point）：提供决策所需的信息

### Google BeyondCorp

Google 的零信任实现（BeyondCorp）：

- 没有"内网"概念
- 所有访问通过公共互联网（加密）
- 基于设备和用户身份的访问控制
- 访问代理（Access Proxy）作为 PEP
